...
author: Michael Cao, Benjamin Qi, Andi Qu, Andrew Wang, Brandon Wang
prerequisites:
  - intro-dp
...
*Proof (Brandon Wang):* Let $f_i$ denote the length of the longest non-increasing subsequence ending at $A_i$. Then all $A_i$'s satisfying $f_i=t$ for a fixed $t$ are an increasing subsequence for each $t$. So we have covered $A$ with (size of longest non-increasing subsequence) increasing subsequences, done. 
